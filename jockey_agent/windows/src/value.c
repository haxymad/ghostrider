...
Value mkint(int64_t i)  { Value v; v.type = JKY_INT;   v.rc = 0; v.v.i = i; return v; }
Value mkfloat(double f) { Value v; v.type = JKY_FLOAT; v.rc = 0; v.v.f = f; return v; }
Value mkbool(int b)     { return mkint(b ? 1 : 0); }
Value mknone(void)      { Value v; v.type = JKY_NONE; v.rc = 0; v.v.i = 0; return v; }
...
void vfree(Value *v) {
    ...
    switch (v->type) {
        ...
        case JKY_DICT:
            ...
            break;
        case JKY_BYTES: free(v->v.bytes.data); break;
        default: break;
    }
    v->type = JKY_NONE;
    v->rc   = 0;
...
Value vcopy(Value v) {
    switch (v.type) {
        case JKY_INT:   return mkint(v.v.i);
        case JKY_FLOAT: return mkfloat(v.v.f);
        case JKY_NONE:  return mknone();
        case JKY_STR:   return mkstr(v.v.s);
        case JKY_ARRAY: {
            Value out = mkarray();
            for (int i = 0; i < v.v.arr->len; i++)
...
Value vshare(Value v) {
    switch (v.type) {
        case JKY_ARRAY: if (v.v.arr)  v.v.arr->rc++;  return v;
        case JKY_DICT:  if (v.v.dict) v.v.dict->rc++; return v;
        case JKY_STR:   return mkstr(v.v.s ? v.v.s : "");
        case JKY_BYTES: {
            Value out;
            out.type = JKY_BYTES;
            out.rc   = 0;
...
static int is_num(Value v) { return v.type == JKY_INT || v.type == JKY_FLOAT; }
static double as_double(Value v) { return v.type == JKY_FLOAT ? v.v.f : (double)v.v.i; }

int value_truthy(Value v) {
    switch (v.type) {
        case JKY_NONE:  return 0;
        case JKY_INT:   return v.v.i != 0;
        case JKY_FLOAT: return v.v.f != 0.0;
        case JKY_STR:   return v.v.s && v.v.s[0] != 0;
        case JKY_ARRAY: return v.v.arr && v.v.arr->len > 0;
        case JKY_DICT:  return v.v.dict && v.v.dict->head != NULL;
        case JKY_BYTES: return v.v.bytes.len > 0;
        default:        return 1;
    }
}
...
char *value_to_str(Value v) {
    char buf[64];
    switch (v.type) {
        case JKY_NONE:  return strdup("none");
        case JKY_INT:
        ...
        case JKY_STR:   return strdup(v.v.s ? v.v.s : "");
        case JKY_ARRAY: {
            size_t cap = 32, n = 0;
            char  *b   = malloc(cap);
            b[n++] = '[';
...
Value value_add(Value a, Value b) {
    if (is_num(a) && is_num(b)) {
        if (a.type == JKY_INT && b.type == JKY_INT) return mkint(a.v.i + b.v.i);
        return mkfloat(as_double(a) + as_double(b));
    }
    if (a.type == JKY_STR || b.type == JKY_STR) {
...
Value value_eq(Value a, Value b) {
    if (is_num(a) && is_num(b)) return mkbool(as_double(a) == as_double(b));
    if (a.type == JKY_NONE || b.type == JKY_NONE)
        return mkbool(a.type == JKY_NONE && b.type == JKY_NONE);
    if (a.type == JKY_STR && b.type == JKY_STR)
        return mkbool(strcmp(a.v.s, b.v.s) == 0);
    if (a.type == JKY_ARRAY && b.type == JKY_ARRAY)
        return mkbool(a.v.arr == b.v.arr);
    if (a.type == JKY_DICT && b.type == JKY_DICT)
...
Value value_neg(Value v) {
    if (v.type == JKY_INT)   return mkint(-v.v.i);
    if (v.type == JKY_FLOAT) return mkfloat(-v.v.f);
...
Value value_len(Value v) {
    switch (v.type) {
        case JKY_STR:   return mkint(v.v.s ? (int64_t)strlen(v.v.s) : 0);
        case JKY_ARRAY: return mkint(v.v.arr ? v.v.arr->len : 0);
        case JKY_DICT: {
            int64_t n = 0;
            if (v.v.dict) for (DictEntry *e = v.v.dict->head; e; e = e->next) n++;
            return mkint(n);
        }
        case JKY_BYTES: return mkint((int64_t)v.v.bytes.len);
        default: return mkint(0);
    }
}

Value value_index(Value obj, Value idx) {
    if (obj.type == JKY_ARRAY && idx.type == JKY_INT) {
        int64_t i = idx.v.i;
        if (i < 0 || i >= obj.v.arr->len) return mknone();
        return vshare(obj.v.arr->items[i]);
    }
    if (obj.type == JKY_STR && idx.type == JKY_INT) {
        int64_t i = idx.v.i;
