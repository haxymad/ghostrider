...
static BuiltinResult bi_list(VM *vm, Value *args, int argc, Value *out) {
    ...
    if (argc == 1 && args[0].type == JKY_ARRAY) {
        *out = vcopy(args[0]);
        return BUILTIN_OK;
    }
    if (argc == 1 && args[0].type == JKY_STR) {
        Value arr = mkarray();
...
static BuiltinResult bi_dict(VM *vm, Value *args, int argc, Value *out) {
    ...
    if (argc == 1 && args[0].type == JKY_DICT) {
        *out = vcopy(args[0]);
        return BUILTIN_OK;
    }
    *out = mkdict();
...
static BuiltinResult bi_range(VM *vm, Value *args, int argc, Value *out) {
    ...
    if (argc == 1) {
        if (args[0].type != JKY_INT) {
            value_set_error("range: int required");
            return BUILTIN_ERROR;
        }
        stop = args[0].v.i;
    } else if (argc == 2) {
        if (args[0].type != JKY_INT || args[1].type != JKY_INT) {
...
static BuiltinResult bi_type(VM *vm, Value *args, int argc, Value *out) {
    ...
    switch (args[0].type) {
        case JKY_INT:   n = "int";    break;
        case JKY_FLOAT: n = "float";  break;
        case JKY_STR:   n = "string"; break;
        case JKY_NONE:  n = "none";   break;
        case JKY_ARRAY: n = "array";  break;
        case JKY_DICT:  n = "dict";   break;
        case JKY_BYTES: n = "bytes";  break;
